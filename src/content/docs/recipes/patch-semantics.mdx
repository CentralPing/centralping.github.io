...
ergo-router enforces valid PATCH content types **after** route matching
(unknown paths return 404, method mismatches return 405) but **before**
the request reaches your route pipeline. This is enabled by default and
configured at router creation time (not per-route):
...
This disables the content-type check that runs after route matching. Your
route pipeline is then responsible for validating the incoming content type.
