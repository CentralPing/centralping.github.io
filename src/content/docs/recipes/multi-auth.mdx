...
- **`undefined`** (not specified) — inherits the router default
- **`false`** — explicitly disables, regardless of default
- **`true`** — uses empty options `{}` (default behavior)
- **`{ ... }`** — overrides the default with custom options

This allows a single `defaults.authorization` to protect the entire
router while individual routes opt out or override with different
strategies.
