...
<Aside type="caution" title="Listen event timing">
  The promise returned by `graceful()` resolves _after_ the server is
  already listening. Do not attach a `'listening'` event listener to
  `server` after `await graceful(...)` — it will never fire because the
  event has already been emitted. The function logs
  `Server listening on {hostname}:{port}` upon successful bind.
</Aside>
