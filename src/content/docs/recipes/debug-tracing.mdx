...
When `debug` is `true`, the pipeline initializes a trace object on the
response accumulator before running middleware:
...
`[fn, setPath]` tuple, or the function name) is pushed onto `steps`.
