...
// In the execute handler, compute elapsed time relative to the start
function computeTiming(req, res, acc) {
  ...
  <TabItem label="Standalone">
    ...
    function tenantResolver(options) {
      ...
      return (req, res, acc) => {
        let cached;
        const resolve = async () => {
          if (!cached) cached = db.tenants.findById(acc.auth?.tenantId);
          return cached;
        };
...
<Aside type="tip">
  The closure-scoped `cached` variable lives only for the duration of one
  request — no module-level `Map` or cleanup logic needed. Each invocation
...
// ✅ If cleanup is needed, attach to res events at factory time
function good() {
  return (req, res) => {
...
function good(options) {
  ...
  return (req, res, acc) => {
    let cached;
    const resolve = async () => {
      if (!cached) cached = db.find(acc.auth?.tenantId);
      return cached;
    };
    return { value: { resolve } };
  };
