...
Built-in ergo middleware factories carry an intrinsic accumulator path
and can be composed as bare functions. Custom middleware that produces
domain data uses a `{fn, setPath}` config object to declare its path
on the domain accumulator:
