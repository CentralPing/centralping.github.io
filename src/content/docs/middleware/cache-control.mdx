---
title: cache-control
description: Cache-Control middleware — pre-computed response header directives with zero per-request cost.
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

Returns pre-computed `Cache-Control` response header tuples. The
directive string is built at factory time, so there is zero per-request
overhead. Accepts either a raw directive string or structured options.
Structured options are validated at construction time. Defaults to the
security-safe `DEFAULT_DIRECTIVES` (`private, no-cache`).

**Pipeline stage:** Cross-cutting

## Import

```js
import {cacheControl} from '@centralping/ergo';
import {DEFAULT_DIRECTIVES} from '@centralping/ergo/http/cache-control';
```

## Options

Pass `directives` as a raw string to bypass structured options and their validation.

| Option | Type | Default | Description |
| ------ | ---- | ------- | ----------- |
| `directives` | `string` | — | Raw directive string (overrides all below; unvalidated) |
| `public` | `boolean` | `false` | Add `public` (mutually exclusive with `private`) |
| `private` | `boolean` | `false` | Add `private` (mutually exclusive with `public`) |
| `noCache` | `boolean` | `false` | Add `no-cache` |
| `noStore` | `boolean` | `false` | Add `no-store` (cannot combine with freshness options) |
| `noTransform` | `boolean` | `false` | Add `no-transform` |
| `mustRevalidate` | `boolean` | `false` | Add `must-revalidate` |
| `proxyRevalidate` | `boolean` | `false` | Add `proxy-revalidate` |
| `immutable` | `boolean` | `false` | Add `immutable` |
| `maxAge` | `number` | — | `max-age` in seconds (non-negative integer) |
| `sMaxAge` | `number` | — | `s-maxage` in seconds (non-negative integer) |
| `staleWhileRevalidate` | `number` | — | `stale-while-revalidate` in seconds (non-negative integer) |
| `staleIfError` | `number` | — | `stale-if-error` in seconds (non-negative integer) |

## Return Value

Always returns response headers. With no structured options, the value is
`DEFAULT_DIRECTIVES`:

```js
{response: {headers: [['Cache-Control', 'private, no-cache']]}}
```

## Error Responses

None. This middleware always injects headers (it does not return HTTP error statuses).

## Construction-time errors

The factory throws `TypeError` when structured options are invalid (raw `directives`
bypass these checks):

| Condition | Error |
| --------- | ----- |
| `maxAge` / `sMaxAge` / `staleWhileRevalidate` / `staleIfError` provided but not a non-negative integer | `TypeError` |
| `public` and `private` both truthy | `TypeError` |
| `noStore` combined with any freshness option above | `TypeError` |

## Usage

<Tabs>
  <TabItem label="Standalone">
    ```js
    import {compose, cacheControl} from '@centralping/ergo';

    // String shorthand
    const pipeline = compose(
      cacheControl({directives: 'public, max-age=3600'}),
    );

    // Structured options
    const pipeline = compose(
      cacheControl({
        private: true,
        maxAge: 0,
        mustRevalidate: true,
      }),
    );
    ```
  </TabItem>
  <TabItem label="ergo-router">
    ```js
    router.get('/static/:id', {
      cacheControl: {public: true, maxAge: 86400, immutable: true},
      execute: (req, res, acc) => ({
        response: {body: staticContent},
      }),
    });
    ```
  </TabItem>
</Tabs>

## RFC References

- [RFC 9111 — HTTP Caching](https://www.rfc-editor.org/rfc/rfc9111)

## API Reference

See the auto-generated [cacheControl API docs](/api/ergo/centralping/ergo/variables/cacheControl/).
