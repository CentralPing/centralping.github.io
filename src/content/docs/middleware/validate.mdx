...
<Aside type="caution" title="ReDoS mitigation">
  Eight format validators use complex regex patterns in full mode that are
  vulnerable to ReDoS with crafted input: `date`, `time`, `date-time`,
  `duration`, `uri`, `uri-reference`, `email`, and `idn-email`. Fast mode
  (the default) uses simplified regexes that validate structure without
  exhaustive semantic checks — for example, fast-mode `date` accepts
  `2026-02-31` because it validates the digit pattern without calendar
  logic. Use `{mode: 'full'}` only when strict RFC compliance is required
  and input sources are trusted.
</Aside>
