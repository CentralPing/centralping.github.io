...
<Aside type="caution" title="ReDoS mitigation">
  Fast mode (the default) uses simplified regexes for 8 format validators
  (`date`, `time`, `date-time`, `iso-time`, `iso-date-time`, `uri`,
  `uri-reference`, and `email`). Full-mode patterns for these formats are
  vulnerable to ReDoS with crafted input. Fast mode validates structure
  without exhaustive semantic checks — for example, fast-mode `date`
  accepts `2026-02-31` because it validates the digit pattern without
  calendar logic. Use `{mode: 'full'}` only when strict RFC compliance is
  required and input sources are trusted.
</Aside>
