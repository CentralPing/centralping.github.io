...
[RFC 9457](https://www.rfc-editor.org/rfc/rfc9457) error body.
...
This example uses [ioredis](https://www.npmjs.com/package/ioredis)
with sorted sets:
...
<Aside type="caution" title="Local counts diverge across instances">
  Each instance tracks its own local counter. The Redis sorted set
  provides cross-instance visibility for monitoring but does **not**
  feed back into the synchronous `hit()` return value. For truly
  shared counters, consider transport-level rate limiting
  (`transport.rateLimit`) or a custom middleware that awaits the store.
</Aside>
...
[`trustProxy`](/recipes/production-deployment/#trust-proxy-configuration) router transport option does
