...
| Condition | Error |
| --------- | ----- |
| `contentSecurityPolicy` / `permissionsPolicy` / HSTS string empty or contains CTL | `TypeError`: `buildSecurityHeaderTuples(): "{name}" option must be a non-empty string without control characters` |
...
| HSTS object missing a non-negative integer `maxAge` | `TypeError`: `buildSecurityHeaderTuples(): "strictTransportSecurity.maxAge" option must be a non-negative integer` |
