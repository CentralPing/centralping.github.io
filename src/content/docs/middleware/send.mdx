...
completes. Reads from both the response accumulator and
[domain accumulator](/concepts/accumulator/) (`acc`) to serialize the
HTTP response.
...
<Aside type="tip" title="Why 'send'?">
  ergo's `send` is a factory wired by the framework — not an imperative
  `res.send()` call inside a handler. The name describes the same concept
  (sending a response) at a different architectural level.
...
| `prefer` | `boolean` | `false` | Read `domainAcc.prefer` for [RFC 7240](https://www.rfc-editor.org/rfc/rfc7240) `return=minimal` / `return=representation`. When `true`, appends `Prefer` to the `Vary` header |
| `paginate` | `boolean` | `false` | Read `domainAcc.paginate` and `responseAcc.paginate` to auto-generate [RFC 8288](https://www.rfc-editor.org/rfc/rfc8288) Link headers and `X-Total-Count` for paginated responses |
| `envelope` | `boolean \| function` | `false` | Wrap 2xx Object bodies in a response envelope |
| `errorFormatter` | `function` | — | Custom error body formatter for 4xx/5xx responses. Receives the [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457) Problem Details object and `{requestId, statusCode, method}` context. Return value becomes the response body as `application/json` instead of `application/problem+json` |
...
None. `send()` writes directly to the HTTP response and is called by
`handler()` after the pipeline completes.
...
For success responses (`statusCode` < 400). Error responses use
[RFC 9457 Problem Details](#error-responses) formatting instead.
