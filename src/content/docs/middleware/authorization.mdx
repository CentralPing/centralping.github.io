...
The response includes a `WWW-Authenticate: Bearer` header and, when
present on `info`, emits `error`, `error_description`, and `error_uri`
challenge attributes from the same-named fields (omitted when unset).
