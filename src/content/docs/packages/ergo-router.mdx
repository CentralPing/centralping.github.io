...
[pipeline builder](/concepts/fast-fail/). Each produces a `{fn, setPath}`
config object that contributes to the domain accumulator.
...
<Aside type="note" title="Independent from responseInfo.duration">
  ...
  Both use `performance.now()` but start from different points — timing
  starts at auto-wrap entry, while `responseInfo.duration` starts at
  the same point but is captured after `send()` completes.
</Aside>
