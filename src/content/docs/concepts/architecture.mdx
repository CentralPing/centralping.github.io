...
- `value` merges into the domain accumulator at the path specified by the
...
Middleware are registered as `[fn, setPath]` tuples that pair the middleware
function with its domain accumulator key:
...
POST/PUT/PATCH), and produces a `[fn, setPath]` tuple array ready for
`compose-with`.
