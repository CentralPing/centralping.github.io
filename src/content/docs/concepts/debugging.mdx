...
```text
Error: Unknown config key "authorisation" in POST /users (did you mean "authorization"?)
```
...
```text
Error: Invalid "authorization" in POST /users: expected object or boolean, got string.
```
...
```text
Error: Route config for POST /users is missing the required "execute" function.
```
