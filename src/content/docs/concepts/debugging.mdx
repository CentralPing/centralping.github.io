...
| `ERGO_IDEMPOTENCY_PROCESSING_EVICTED` | `idempotency()` store | All entries are `processing` and a new entry must be stored — oldest `processing` entry was evicted | Increase `maxKeys` to match peak concurrent request volume, or use a store with LRU eviction that protects `processing` entries |
